...
func newProxyServer(ctx context.Context, config *kubeproxyconfig.KubeProxyConfiguration, master string, initOnly bool, flagzReader flagz.Reader) (*ProxyServer, error) {
	...
	supported, dualStackSupported := backend.CheckIPFamilySupport(ctx, s.PrimaryIPFamily)
	if !supported {
		return nil, fmt.Errorf("no backend support for primary IP family %s", s.PrimaryIPFamily)
	}
