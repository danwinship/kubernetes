...
func ResolveEndpoint(services listersv1.ServiceLister, endpointSlices EndpointSliceGetter, namespace, id string, port int32) (*url.URL, error) {
	...
	host := RandomEndpoint(slices, svcPort.Name, preferredIPFamily)
	if host == "" {
...
// in slices that serves svcPortName, preferably of preferredIPFamily. If there are no
// suitable endpoints it will return "".
func RandomEndpoint(slices []*discoveryv1.EndpointSlice, svcPortName string, preferredIPFamily v1.IPFamily) string {
	if len(slices) > 1 {
