...
func ClearEntriesForPort(execer exec.Interface, port uint16, isIPv6 bool, protocol v1.Protocol) error {
	if port == 0 {
		return fmt.Errorf("bad port number. The port number must be greater than zero")
	}
	parameters := parametersWithFamily(isIPv6, "-D", "-p", protoStr(protocol), "--dport", strconv.Itoa(int(port)))
	err := Exec(execer, parameters...)
	if err != nil && !strings.Contains(err.Error(), NoConnectionToDelete) {
...
func ClearEntriesForPortNAT(execer exec.Interface, dest string, port uint16, protocol v1.Protocol) error {
	...
	parameters := parametersWithFamily(utilnet.IsIPv6String(dest), "-D", "-p", protoStr(protocol), "--dport", strconv.Itoa(int(port)), "--dst-nat", dest)
	err := Exec(execer, parameters...)
	if err != nil && !strings.Contains(err.Error(), NoConnectionToDelete) {
		return fmt.Errorf("error deleting conntrack entries for UDP port: %d, error: %v", port, err)
	}
